...
populateShinyApp <- function(shinyDirectory,
  ...
  if(missing(shinyDirectory)){
    shinyDirectory <- system.file("shiny", "PLPViewer", package = "Covid19SeverityPredictionwWithoutCV")
  }
...
viewShiny <- function(package = NULL){
  if(is.null(package)){
    appDir <- system.file("shiny", "PLPViewer", package = "Covid19SeverityPredictionwWithoutCV")
  }
  if(!is.null(package)){
