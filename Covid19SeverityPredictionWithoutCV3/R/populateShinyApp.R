...
populateShinyApp <- function(shinyDirectory,
  ...
  if(missing(shinyDirectory)){
    shinyDirectory <- system.file("shiny", "PLPViewer", package = "Covid19SeverityPredictionWithoutCV3")
  }
...
viewShiny <- function(package = NULL){
  if(is.null(package)){
    appDir <- system.file("shiny", "PLPViewer", package = "Covid19SeverityPredictionWithoutCV3")
  }
  if(!is.null(package)){
